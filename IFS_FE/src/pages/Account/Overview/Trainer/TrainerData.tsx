...
const TrainerData = ({ user }) => {
  ...
  const trainerTrainings = trainings
    .filter((training) => training.instructorId === user.id)
    .filter(
      (training) => new Date(training.date).getDate() <= currentDate.getDate()
    )
    .filter((training) => training.visitors.length >= 2);
    ...
    <div className="trainer-overview">
      <div className="results-section card-el-bg ">
        ...
        <div className="result-el">
          ...
          <span className="bold green-clr">{trainerTrainings.length}</span>
        </div>
      </div>
              ...
              <div key={training.id} className="training-element">
                ...
                <div className="training-card acc-card-el-bg2">
                  <div>
                    ...
                    <span className="status4 xs-font">Group Class</span>
                  </div>
                  <div>
      ...
      <div className="history-section card-el-bg">
        ...
        <div className="history">
          {trainerTrainings.map((training) => (
            <HistoryElement key={training.id} data={training} trainer={true} />
          ))}
        </div>
      </div>
    </div>
