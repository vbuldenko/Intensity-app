...
export const updateReservation = async (abonementId, trainingId, userId, updateType) => {
...
const handleReservation = async (abonement, training) => {
    ...
    if (isTrainingReserved(abonement, training)) {
        throw ApiError.BadRequest('Already reserved: You have already reserved your place!');
    }
...
const handleCancellation = async (abonement, training) => {
    ...
    abonement.visitedTrainings = removeTraining(abonement.visitedTrainings, training._id);
...
const handleReturn = async (abonement, trainingId, userId) => {
    ...
    // Remove the training from visitedTrainings and user from visitors
    abonement.visitedTrainings = removeTraining(abonement.visitedTrainings, trainingId);
...
export const cancelNotHeldTrainings = async (abonementId) => {
    ...
    try {
        for (const training of abonement.visitedTrainings) {
            const canProceed = canTrainingProceed(training.date, training.visitors.length);
            if (!canProceed) {
...
const isTrainingReserved = (abonement, training) => {
    return abonement.visitedTrainings.some((visitedTraining) => visitedTraining._id.toString() === training._id.toString());
};
