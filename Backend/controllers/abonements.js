    ...
    try {
        ...
        if (user.role !== "client") {
            return response.status(401).json({
                error: "Unauthorized: You are not in client role! (1)",
            });
        }
        ...
        if (user.id !== abonement.user.toString()) {
            return response.status(400).json({ error: "not your abonement" });
        }
        ...
        if (updateType === "reservation") {
            ...
            if (abonement.left === 0) {
                abonement.status = "ended"; //change status of abonement
            }
            abonement.history.push(body.trainingId); //wy not _id?
        } else if (updateType === "cancellation") {
            if (abonement.left === 0) {
                abonement.status = "active"; //change status of abonement
            }
